...
fb_pb_blind_bot_q = fb_pb_blind('text_base_blind_ditect_dataset.json', model_name="text_base_blind_ditect_dataset_model")
fb_pb_blind_bot_q.load_model()

interviewer = GenericAssistant('text_base_command_dataset.json', model_name="text_base_command_model")
    ...
    while False:
        ...
        if camera_open:
            ...
            if qindexpreviosRef != qindexglobalRef:
                ...
                socketio.emit('stream three q4 game', {'testnameglobalRef': testnameglobalRef, 'testlevelglobalRef': testlevelglobalRef, 'qindexglobalRef': qindexglobalRef , 'answerText' :answerText}, to=camera_room_contents)
            ...
            socketio.emit('cam_image', {'image': frame_encoded.decode('utf-8')})
            time.sleep(0.2)
...
def gesture_game_quiz(np_frame , rightAnswer):
    ...
    isAnswerOk = 1 #! 0 or 1 depeneding on the frame 
...
def handle_user_message(message):
    ...
    if msg['messageType'] == 'audio':
        ...
        responseSave = responseLive = generate_response_for_audio(msg,user_id, audio_base64msg)
        pass
...
def handle_stream(data):
    ...
    if data['nameOftheTest'] == "StreamThree" and data['levelofmcq'] == "q4":
        ...
        answerText , mark , aireply , streamNumber , isAnswerOk =  gesture_game_quiz(np_frame , data['rightAnswer'])
    ...
    #* Final quiz
        ...
        emotionScore =  emotion_score(np_frame)
                    ...
                    # Check if the key already exists in the dictionary
                        if nameOftheTest in users_data[i]:
                            ...
                            if nameOftheTest == "AIOneQuection":
                                ...
                                variables = [('StreamOne',  users_data[i]["stream_1_selection_points"]), ('StreamTwo',  users_data[i]["stream_2_selection_points"]), ('StreamThree',  users_data[i]["stream_3_selection_points"])]
                                ...
                                if variables[0][1] == variables[1][1]:
                                    ...
                                    winner_variables = [variable[0] for variable in variables[:2]]
                                ...
                                if isinstance(winner_variables, list):
                                    print(f"The winners are {', '.join(winner_variables)}")
                                    
                                    selected_stream = random.choice(winner_variables)
                                else:
    ...
    #! stream mcqs
        ...
        if nameOftheTest == "StreamOne":
            ...
            if levelofmcq == "q1":
                ...
                return answerText , mark , aireply
    ...
    try:
        ...
        if os.path.exists(temp_folder_path):
            ...
            if os.path.exists(path_audio):
                ...
                answerProceesed , mark , aireply , streamNumber= process_speech_ai_ques(nameOftheTest, levelofmcq , rightanswer,path_audio,userspeachtotext)
        ...
        if nameOftheTest == "AIOneQuection":
            ...
            if int(qindex_temp) != 0:
                try:
                    ...
                    print('try acsept - stream', stream , '     detels - ',detels)
    ...
    try:
        ...
        if os.path.exists(temp_folder_path):
            ...
            if os.path.exists(path_audio):
                ...
                answerProceesed , mark , aireply , streamNumber , isNextQ = process_speech_ai_interviewone(nameOftheTest, levelofmcq , rightanswer,path_audio,qindex_temp,name,id)
